...
let slideIndeXDown = 0;
showSliderDown();

function showSliderDown() {
  let n;
  let slider = document.getElementsByClassName("mySliderDown");
  let dots = document.getElementsByClassName("dot");
  for (n = 0; n < slider.length; n++) {
    slider[n].style.display = "none";  
  }
  slideIndeXDown++;
  if (slideIndeXDown > slider.length) {slideIndeXDown = 1}    
  for (n = 0; n < dots.length; n++) {
    dots[n].className = dots[n].className.replace(" active", "");
  }
  slider[slideIndeXDown-1].style.display = "block";  
  dots[slideIndeXDown-1].className += " active";
  setTimeout(showSlider, 7000); // Change image every 2 seconds
}
...
const carouselmySliderDown = document.querySelector(".slideshow-containerDown");
firstImg = carousel.querySelectorAll("img")[0];
arrowIcon = document.querySelectorAll(".wapperhanghoa i");

let isDragStartmySliderDown = false;
let frittImgWidthmySliderDown = firstImg.clientWidth;

arrowIcon.forEach(icon => {
  icon.addEventListener("click", () => {
    console.log(icon);
    const speedFactor = 4; // Hệ số tốc độ
    carouselhai.scrollLeft += icon.id === "left" ? -frittImgWidthmySliderDown * speedFactor : frittImgWidthmySliderDown * speedFactor;
  });
});
